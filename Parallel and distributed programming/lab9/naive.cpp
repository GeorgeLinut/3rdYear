...
inline void thread_work(int st, int dr, const vector <int> &a, const vector <int> &b, vector <int> &res) {
  cerr << ">>> thread work " << st << ' ' << dr << "\n";
  for(int i = st; i < dr; ++ i) {
...
int main(int argc, char** argv) {
  ...
  if (me == 0) {
    generate(a, b, n);
    fprintf(stderr, ">>> master says: input generated\n");
    fprintf(stderr, ">>> master says: sending work to slaves\n");
