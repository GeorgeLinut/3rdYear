...
void brute_force(int *a,int *b,int *res,int n){
	for (int i=0;i<2*n;++i){
		res[i]=0;
	}
	for (int i=0;i<n;++i){
...
inline void work(int st,int dr, vector<int>&a,vector<int>&b,vector<int>&result){
	cerr << "> do work do karatsuba " << st << ' ' << dr << "\n";
...
inline void slave(int me) {
  ...
  work(st, dr, a, b, result);
  MPI_Bsend(result.data(), 2 * n - 1, MPI_INT, 0, 5, MPI_COMM_WORLD);
...
int main (int argc, char* argv[]){
  ...
  if(me == 0){
    ...
    work(st,dr,aux,b,result);
    collect(n,nrProcs,result);
    result.resize(2*n-1);
    check(a,b,res);
  }
  else{
